class CarDealership {
    ...
    currentCar (){
        ...
        this.availableCars.forEach( car => result+=`---${car.model} - ${car.horsepower} HP - ${car.mileage.toFixed(2)} km - ${car.price.toFixed(2)}$`);
        return result;
    };
    salesReport (criteria){
        ...
        let print = `-${this.name} has a total income of ${this.totalIncome.toFixed(2)}$`;
        print+= `\n-${this.soldCars.length} cars sold:`;
        this.soldCars.forEach( car => print+=`\n---${car.model} - ${car.horsepower} HP - ${car.price.toFixed(2)}$`);
        return print;
    };
}
